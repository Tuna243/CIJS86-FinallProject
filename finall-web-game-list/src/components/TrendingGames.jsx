        ...
        {loading?
            gameList.map((item,index)=>index < 4 &&(
                ...
                hover:scale-110 transition-all duration-300 ease-in-out'>
                    ...
                    <button className='relative bottom-0 bg-[#2563eb] border-b-2 text-white rounded-lg w-30 h-30 mb-2 p-2 hover:bg-[#25eb77] 
                    '        onClick={() => navigate(`/Games/Details/${item.id} `)}>SEE MORE</button>
                </div>
            ))
        :<Loading/>}
    </div>
